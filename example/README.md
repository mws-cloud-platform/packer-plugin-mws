...
- `iops` - IOPS for the disk (defaults to 1000).
...
The example includes simple `Hello!` provisioner. You can customize provisioners based on your needs.
...
1. Update the configuration values in `build.pkr.hcl` with your actual values, most important to replace are:
    ...
    * source_image (mws-ubuntu image in example can be deprecated)
