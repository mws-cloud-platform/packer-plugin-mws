...
func (s *StepCreateSignedLink) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	awsClient := state.Get(common.AWSClientKey).(AWSClient)

	ui.Say("Creating presigned URL for Object Storage object...")

	bucket, key, found := strings.Cut(s.Path, "/")
	if !found {
		return common.ActionHaltWithErrorf(state, "split object_storage_path into bucket and key: %s", s.Path)
	}
	...
	if err != nil {
		return common.ActionHaltWithErrorf(state, "create presigned URL: %w", err)
	}

	ui.Sayf("Presigned URL created: %s", presignResult.URL)

	state.Put(common.ExternalURLKey, presignResult.URL)
